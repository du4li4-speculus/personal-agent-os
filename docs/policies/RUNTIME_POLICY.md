...
4. Declared artifacts must exist, be non-empty, stay within the current run, and pass declared generic/domain validation gates.
5. Loading, execution, validation, recovery, and failure are represented truthfully in the trace.
6. Runtime writes no persistent Memory.
